...
function openApp(appId) {
  ...
  if (cameraApp) {
    const video = cameraApp.querySelector('.camera-preview');
    const status = cameraApp.querySelector('.camera-status');
    const startButton = cameraApp.querySelector('.camera-start');
    const snapButton = cameraApp.querySelector('.camera-snap');
    const snapshot = cameraApp.querySelector('.camera-snapshot');
    let stream;
    startButton?.addEventListener('click', async () => {
      if (!navigator.mediaDevices?.getUserMedia) {
        status.textContent = 'Camera is not supported in this browser.';
        return;
      }
      try {
    ...
    snapButton?.addEventListener('click', () => {
      ...
      try {
        ...
        status.textContent = 'Failed to capture snapshot.';
      }
    });
    if (!windowElement.cleanup) windowElement.cleanup = [];
    windowElement.cleanup.push(() => {
      if (stream) {
  ...
  if (browserFrame && addressForm) {
    ...
    const navigate = () => {
      ...
      const validatedUrl = validateUrl(address);
      if (!validatedUrl) {
        console.warn('Invalid URL:', address);
        return;
      }
    ...
    windowElement.querySelector('.browser-back')?.addEventListener('click', () => {
      try { browserFrame.contentWindow.history.back(); } catch (err) { console.warn('History back failed:', err); }
    });
    windowElement.querySelector('.browser-forward')?.addEventListener('click', () => {
      try { browserFrame.contentWindow.history.forward(); } catch (err) { console.warn('History forward failed:', err); }
    });
    windowElement.querySelector('.browser-reload')?.addEventListener('click', () => {
      try { browserFrame.contentWindow.location.reload(); } catch (err) { console.warn('Reload failed:', err); }
    });
    windowElement.querySelector('.external-link')?.addEventListener('click', () => {
      const url = addressInput?.value?.trim();
      if (url) window.open(url, '_blank', 'noopener,noreferrer');
    });
    browserFrame.addEventListener('load', () => {
      const msgEl = windowElement.querySelector('.browser-message');
      if (msgEl) msgEl.hidden = true;
    });
    browserFrame.addEventListener('error', () => {
      console.error('Browser frame error');
    });
  }
  ...
  if (wallpaperApp) {
    ...
    const handleFile = (file) => {
      ...
      if (!file.type.startsWith('image/')) {
        statusEl.textContent = 'Please pick an image file (PNG, JPG, WebP).';
        return;
      }
      if (file.size > 4.5 * 1024 * 1024) {
        statusEl.textContent = 'Image is large (>4.5MB). Try compressing or use a URL instead.';
      }
      fileStatus.textContent = `${sanitize(file.name)} — ${(file.size/1024).toFixed(0)} KB`;
      ...
      reader.onload = () => {
        try {
          ...
          syncWallpaperAppPreview?.();
          wallpaperApp.querySelectorAll('.wallpaper-preset').forEach(el => el.classList.remove('active'));
          statusEl.textContent = `Wallpaper set from file. Saved locally.`;
          fileStatus.textContent = 'Saved — will persist after reload.';
        } catch (err) {
          console.error('Wallpaper save error:', err);
          statusEl.textContent = 'Could not save that image locally.';
        }
      };
      reader.onerror = (err) => {
        console.error('FileReader error:', err);
        statusEl.textContent = 'Failed to read that file.';
      };
    ...
    urlBtn?.addEventListener('click', () => {
      ...
      if (!url) {
        statusEl.textContent = 'Paste an image URL first.';
        return;
      }
      // Validate URL
      const validUrl = url.startsWith('data:image/') ? url : validateUrl(url);
      if (!validUrl) {
        statusEl.textContent = 'Invalid URL. Use https:// or paste a data URL.';
        return;
      }
      // test load
      const img = new Image();
      const loadTimeout = setTimeout(() => {
        statusEl.textContent = 'Image load timeout. Try another URL.';
      }, 8000);
      img.onload = () => {
        clearTimeout(loadTimeout);
        applyWallpaper({ type: 'image', value: validUrl, fit: wallpaperState.fit, blur: wallpaperState.blur, dim: wallpaperState.dim, presetId: null });
        syncWallpaperAppPreview?.();
        wallpaperApp.querySelectorAll('.wallpaper-preset').forEach(el => el.classList.remove('active'));
      ...
      img.onerror = (err) => {
        ...
        statusEl.textContent = 'Could not load that URL. Check link or try another image.';
      };
      img.src = validUrl;
...
(function setupDesktopWallpaperDrop() {
  ...
  desktop.addEventListener('drop', (e) => {
    ...
    // Validate file size before reading (max 5MB)
    const maxSize = 5 * 1024 * 1024;
    if (file.size > maxSize) {
      const openWallpaper = openWindows.get('wallpaper');
      const status = openWallpaper?.querySelector('#wallpaperStatus');
      if (status) status.textContent = `File too large (${(file.size / 1024 / 1024).toFixed(1)}MB > 5MB). Try a smaller file.`;
      else alert(`Image too large. Max 5MB, got ${(file.size / 1024 / 1024).toFixed(1)}MB.`);
      return;
    }
    ...
    reader.onload = () => {
      try {
        ...
        if (st) st.textContent = `Dropped wallpaper applied: ${sanitize(file.name)}`;
      } catch (err) {
        console.error('Wallpaper file error:', err);
        const openWallpaper = openWindows.get('wallpaper');
        const status = openWallpaper?.querySelector('#wallpaperStatus');
        if (status) status.textContent = 'Error loading wallpaper. Try a different file.';
      }
    };
    reader.onerror = (err) => {
      console.error('FileReader error:', err);
...
(function initDoraemon(){
  ...
  function fly(){
    ...
    if(window.anime){
      ...
      // More realistic flight path with wobble and tilt
      anime({
        targets: dora,
        left: [startX, startX + (flyDir * 40), targetX - (flyDir * 30), targetX],
        bottom: [14, 80, 160, 140, 14],
        ...
        complete: ()=>{
          ...
          x = targetX; dora.style.left = x + 'px'; dora.style.rotate = '0deg';
          doJump(); showBubble(phrases[Math.floor(Math.random()*phrases.length)], 3000);
        }
      });
      // side-to-side wobble for wind effect - tilt and sway
      anime({
        targets: dora,
        rotate: [0, -2.5, 1.8, -2.2, 1.5, -1.2, 0.8, -0.5, 0],
        duration: flightDuration,
        easing: 'easeInOutSine'
      });
      // propeller wind effect - shadow scale with wobble
      anime({ targets: '.dora-shadow', scale:[1,.45,1], duration:flightDuration, easing:'easeInOutSine' });
    } else {
